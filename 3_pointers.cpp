 ...
 * */
 	cout << "\n-----------------------------------------------------------------------------" <<endl;
 	cout << "\nPassando uma pointeiro de inteiro para uma variável e multiplicando por 2: " <<endl;
	double_pointer(number_pointer);
	
/* Pointers também podem ser retornadas por funções
 **/ 
	cout << "\n-----------------------------------------------------------------------------" <<endl;
	int *meu_array = create_array(10, 5);
	display_array(meu_array, 10);
	delete [] meu_array;
...
// Cria um pointer nulo e aloca memória de forma dinâmica
	int *new_storage = nullptr;
	new_storage = new int[size];
	
// Itera de forma a preencher a memória alocada com o valor
	for (size_t i{0}; i < size; ++i)
		new_storage[i] = init_value;
	return new_storage;
}

// Exibe valores no array
void display_array(const int *const array, size_t size) {
	for (size_t i = 0; i < size; ++i)
		cout << array[i] << " ";
	cout << endl;
